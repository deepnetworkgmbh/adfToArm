...
namespace AdfToArm.Tests.Dataset
{
    ...
    {
        private const string FullFilePath = @"./samples/pipelines/copy/sqlprocsource_blobSink_full.json";
        private const string MinFilePath = @"./samples/pipelines/copy/sqlquerysource_blobSink_min.json";
        ...
        public void AdfSerializer_ShouldParse_AllProperties()
        {
            ...
            var sink = props.Sink.ShouldBeAssignableTo<CopySinkBlob>();
            sink.Type.ShouldBe(CopySinkType.BlobSink);
            sink.CopyBehavior.ShouldNotBe(CopyBehavior.PreserveHierarchy, "CopyBehavior should not be a default one");
            sink.WriteBatchSize.ShouldNotBeNull();
            sink.WriteBatchTimeout.ShouldNotBeNull();
            sink.BlobWriterAddHeader.ShouldNotBeNull();
        }
        ...
        public void AdfSerializer_ShouldParse_MinSetOfProperties()
        {
            ...
            var sink = props.Sink.ShouldBeAssignableTo<CopySinkBlob>();
            sink.Type.ShouldBe(CopySinkType.BlobSink);
            sink.CopyBehavior.ShouldBeNull();
            sink.WriteBatchSize.ShouldBeNull();
            sink.WriteBatchTimeout.ShouldBeNull();
            sink.BlobWriterAddHeader.ShouldBeNull();
        }
    }
}
