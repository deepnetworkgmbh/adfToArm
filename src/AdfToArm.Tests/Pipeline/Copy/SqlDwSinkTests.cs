...
{
    [TestClass]
    public class SqlDwSinkTests
    {
        private const string FullFilePath = @"./samples/pipelines/copy/blobSource_sqldwSink_full.json";
        private const string MinFilePath = @"./samples/pipelines/copy/blobSource_sqldwSink_min.json";
        ...
        public void AdfSerializer_ShouldParse_MinSetOfProperties()
        {
            ...
            sink.AllowPolyBase.ShouldBeNull();
            sink.SqlWriterCleanupScript.ShouldBeNull();
            sink.PolyBaseSettings.ShouldBeNull();
        }
    }
}
