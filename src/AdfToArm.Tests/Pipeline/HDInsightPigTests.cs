...
{
    [TestClass]
    public class HDInsightPigTests
    {
        private const string ScriptInlineFile = @"./samples/pipelines/activity_hdi_pig_inline.json";
        private const string ScriptPathFile = @"./samples/pipelines/activity_hdi_pig_path.json";
        ...
        public void AdfSerializer_ShouldParse_ScriptPath()
        {
            ...
            activity.Type.ShouldBe(ActivityType.HDInsightPig);
            activity.Outputs.ShouldNotBeEmpty();
            activity.LinkedServiceName.ShouldNotBeNullOrWhiteSpace();

            var props = activity.TypeProperties.ShouldBeAssignableTo<HDInsightPigTypeProperties>();
